#!/usr/bin/env python3
"""Generate IngramSpark paperback cover PDF for From the Beginning.

IngramSpark specs (5.5x8.5 perfect bound paperback, B&W white paper, 154 pages):
  Trim size: 5.5" x 8.5"
  Spine width: 154 x 0.002252 = 0.3468" ~ 0.347"
  Bleed: 0.125" on all outside edges (not on spine edges)
  Total document width: 0.125 + 5.5 + 0.347 + 5.5 + 0.125 = 11.597"
...
OUTPUT = BOOK_DIR / "FromTheBeginning_IngramSpark_Paperback_Cover.pdf"
BG_IMAGE = BOOK_DIR / "FromTheBeginning_Portrait.png"
...
# --- Spine calculation ---
PAGE_COUNT = 154
SPINE_W = round(PAGE_COUNT * 0.002252, 3)  # 0.347"
...
def draw_spine(c):
    """Draw spine text. At 0.347" we can fit small text."""
    c.saveState()
    ...
    c.drawCentredString(0, 2.5, "From the Beginning")
...
def draw_back_cover(c):
    ...
    # --- ISBN placeholder (lower-right of back cover safe area) ---
    # Leave a white rectangle where the barcode will go once we have an ISBN
    bc_w = 1.75 * inch
    bc_h = 0.95 * inch
    pad = 0.08 * inch
    box_w = bc_w + 2 * pad
    ...
    c.drawCentredString(box_x + box_w / 2, box_y + box_h / 2 - 3, "ISBN BARCODE")
    c.drawCentredString(box_x + box_w / 2, box_y + box_h / 2 - 14, "PLACEHOLDER")
...
def main():
    ...
    print('Generating IngramSpark PAPERBACK cover PDF for "From the Beginning"...')
    ...
    c.setTitle("From the Beginning - IngramSpark Paperback Cover")
