#!/usr/bin/env python3
"""Generate HTML chapter files + index + preface/epilogue/part pages
for 'Why Do You Delay?'.

Theme: Warm gold (#C69B56, matches cover) + river blue (#6A8899).

Output files (all in this book directory):
  index.html           — book landing page with chapter grid
  preface.html         — preface page
  part-1.html, part-2.html, part-3.html — part intro pages
  chapter-01.html through chapter-13.html — chapter pages
  epilogue.html        — epilogue page
"""
...
# Theme (matches the cover: warm gold + muted river blue-grey)
ACCENT_PRIMARY       = "#C69B56"
ACCENT_SECONDARY     = "#6A8899"
ACCENT_PRIMARY_RGB   = "198, 155, 86"
ACCENT_SECONDARY_RGB = "106, 136, 153"

PROGRESS_KEY = "whyDoYouDelay_progress"

# Landing-page intro blurb (prose, 3-4 short paragraphs)
INTRO_PARAGRAPHS = [
    ...
    "Scriptures actually say — not on what we have always assumed "
    "they say, and not on what we have been taught by men.",
...
# ============================================================================
# NAV DROPDOWN (shared across all reading pages)
# ============================================================================

def build_chapter_select(current_key, book):
    """Build the chapter dropdown.

    current_key is one of:
      "preface"          - on the preface page
      ("part", p_idx)    - on a part intro page
      ("chapter", num)   - on a chapter page
      "epilogue"         - on the epilogue page
      None               - on the index (no current)
    """
    ...
    sel = ' selected' if current_key == "epilogue" else ''
    options.append(f'<option value="epilogue.html"{sel}>'
                   f'Epilogue: {book["epilogue_title"]}</option>')
    return "\n          ".join(options)
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body::before {{
      ...
      background:
        ...
        radial-gradient(circle at bottom, rgba({ACCENT_SECONDARY_RGB},0.05), transparent 50%);
      pointer-events: none;
    }}
    .glass-page-wrapper {{
...
def _page_frame(title, inner_html, chapter_select, prev_link, next_link,
                ...
                prev_kbd, next_kbd):
    """Common wrapper: nav, header, content, mark-complete, footer."""
    if next_kbd:
...
def build_preface_html(book):
    ...
    """
    first_part_num = book["parts"][0]["chapters"][0]["num"] if book["parts"] else 1
...
def build_part_html(part, p_idx, book):
    ...
    # prev: preface (for part 1), or last chapter of previous part
    if p_idx == 1:
    ...
    # next: first chapter of this part
...
def build_chapter_html(ch, book):
    ...
    # prev: previous chapter, OR this part's intro page if it's the first
    # chapter of a part, OR preface if it's chapter 1.
    if idx == 0:
        prev_url = "part-1.html"
        prev_label = "Part One"
    else:
        prev_ch = all_chapters[idx - 1]
        # If previous chapter is in a different part, go to current part's intro page
        # Determine which part this chapter lives in
    ...
    # next: next chapter's part intro (if crossing parts) or the chapter itself,
    # OR the epilogue if it's the last chapter.
    if idx == len(all_chapters) - 1:
        next_url = "epilogue.html"
        next_label = "Epilogue"
    else:
        ...
        if part_of_next != part_of_this:
            # Jump to next part's intro page
...
def _part_index_of(book, chapter_num):
    """1-based part index of a given chapter number."""
    for p_idx, part in enumerate(book["parts"], start=1):
        for ch in part["chapters"]:
...
def build_epilogue_html(book):
    content = md_body_to_html(book["epilogue_md"])
    inner = f"""
      <header>
        <p class="chapter-num">Epilogue</p>
        <h1>{book['epilogue_title']}</h1>
      </header>
      <div class="content">{content}</div>
    """
    chapter_select = build_chapter_select("epilogue", book)
    last_ch = book["chapters"][-1]["num"]
    prev_url = f"chapter-{last_ch:02d}.html"
    prev_link = f'<a href="{prev_url}">&larr; Chapter {last_ch}</a>'
    next_link = '<a class="disabled">The End</a>'
    return _page_frame(
        title=f"Epilogue: {book['epilogue_title']} | {TITLE}",
        inner_html=inner,
        chapter_select=chapter_select,
        prev_link=prev_link,
        next_link=next_link,
        progress_key_name=PROGRESS_KEY,
        progress_id="epilogue",
        mark_complete_label="Mark Epilogue Complete",
        prev_kbd=prev_url,
        next_kbd="",
    )
...
def build_index_html(book):
    ...
    # Part sections — each with an intro card + chapter cards
    part_sections = ""
    ...
    # Epilogue card
    epilogue_card = f"""
      <a href="epilogue.html" class="lesson-card" data-epi>
        <span class="lesson-num">Epilogue</span>
        <div class="lesson-title">{book['epilogue_title']}</div>
        <span class="progress-dot" id="dot-epilogue"></span>
      </a>
    """
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .progress-dot.visited {{
      border-color: var(--accent); background: rgba({ACCENT_PRIMARY_RGB},0.3);
    }}
  ...
  <div class="glass-page-wrapper">
    <div class="glass-page-inner">
      ...
      <div class="hero-cover">
        <img src="cover_front.jpg" alt="{TITLE} &mdash; book cover" loading="eager">
      </div>
...
{intro_html}
      ...
      <section class="chapter-section">
        ...
        <div class="lesson-grid">{epilogue_card}
        </div>
      </section>
  ...
  <script>
    ...
    function updateProgressUI() {{
      ...
      applyDot('dot-part1', p['part1']);
      applyDot('dot-part2', p['part2']);
      applyDot('dot-part3', p['part3']);
      applyDot('dot-epilogue', p['epilogue']);
...
def main():
    ...
    print(f"  Theme: Warm Gold ({ACCENT_PRIMARY}) + Muted River Blue ({ACCENT_SECONDARY})")
    ...
    # Epilogue
    print("  Writing epilogue.html")
    (BOOK_DIR / "epilogue.html").write_text(build_epilogue_html(book), encoding='utf-8')

    print(f"\nDone. Wrote index + preface + {len(book['parts'])} parts + "
          f"{len(book['chapters'])} chapters + epilogue.")
