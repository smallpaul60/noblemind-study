...
# Replacements are case-sensitive and applied as whole words.
NAME_REPLACEMENTS = {
    # -------------------------------------------------------------------------
    # BOOK-SPECIFIC — add/remove per book
    # -------------------------------------------------------------------------
    # Common patterns from past books (uncomment as needed):
    #
    ...
    #   "Job":       "Jobe",         # avoids "job" (employment)
    #   "Boaz":      "Bo-az",        # avoids "boats"
    #   "Orpah":     "Or-pah",       # avoids "Oprah"
    #   "Salmon":    "Sal-mone",     # avoids the fish
    #   "Haman":     "Hay-man",
    #
    # Other names that get mangled:
    #   "Hagar":     "Hay-gar",
    #   "Obed":      "O-bed",
    #   "Rahab":     "Ray-hab",
    #   "Susa":      "Soo-sah",
...
# =============================================================================
# Reference terminal-period enforcement
# =============================================================================
# Print citations like "— **Genesis 1:1, NASB**" end with bold-close, no period,
# because the visual style and line break do the sentence-ending work for
# print readers. In audio, a TTS engine treats the line break as a soft pause
# and runs the reference into the next sentence. Every reference at the end
# of a line needs a terminal period.
#
# This regex matches a converted reference (ends with a number-word) at the
# end of a line and adds a period if one isn't already there. It runs AFTER
# all reference conversions so it sees the spelled-out form.
NUMBER_WORDS_RE = (
    r"(?:zero|one|two|three|four|five|six|seven|eight|nine|"
    r"ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|"
    r"twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred)"
)
# Match: "verse <words>", "verses <words> and <words>", "verses <words> through <words>",
# or "chapters <words> through <words>" — followed by end of line (no period yet).
# The trailing [a-z]? catches verse-letter suffixes like "23a" -> "twenty-threea".
REFERENCE_TERMINAL_PATTERN = re.compile(
    rf"((?:verse|verses|chapters?) [\w\s-]*?{NUMBER_WORDS_RE}[a-z]?)$",
    re.MULTILINE | re.IGNORECASE
)


def ensure_reference_periods(text):
    """Add a terminal period after any reference that ends a line without one."""
    return REFERENCE_TERMINAL_PATTERN.sub(r"\1.", text)
...
# =============================================================================
# Translation tag stripping
# =============================================================================
# Bible quotes in NobleMind manuscripts end with ", NASB" (or occasionally
# another translation abbreviation) for print readers. In audio, repeating
# the translation after every quote is noise — listeners only need to be
# told the translation once, in the front matter or dedication.
#
# This strips the trailing translation tag from references but leaves any
# in-prose mention of a translation alone (e.g., "we use the NASB throughout"
# stays intact because it lacks the leading comma).
TRANSLATION_TAGS = (
    "NASB|ESV|KJV|NKJV|NIV|NLT|NRSV|RSV|ASV|CSB|HCSB|NET|MSG|AMP"
)
TRANSLATION_TAG_PATTERN = re.compile(
    rf",\s*\*?\*?(?:{TRANSLATION_TAGS})\*?\*?",
    re.UNICODE
)


def strip_translation_tags(text):
    """Remove ', NASB' and similar trailing translation abbreviations from quotes.

    Also cleans up orphaned bold/italic markers left behind. The full
    print citation pattern is '**Genesis 1:1, NASB**' — stripping ', NASB**'
    leaves '**Genesis 1:1' with an unclosed bold marker, so we close it.
    """
    text = TRANSLATION_TAG_PATTERN.sub("", text)
    # Heal any unclosed bold markers on the same line (citations live on one line)
    # Pattern: opening ** ... no closing ** before end of line
    def heal_line(m):
        line = m.group(0)
        # Count ** markers; if odd, the citation got truncated — close it
        if line.count("**") % 2 == 1:
            line = line.rstrip() + "**"
        return line
    text = re.sub(r"^.*\*\*.*$", heal_line, text, flags=re.MULTILINE)
    return text
...
def convert(text):
    ...
    text = strip_translation_tags(text)                            # 1. strip ", NASB" tags
    text = process_headings(text)                                  # 2. headings
    text = strip_markdown(text)                                    # 3. strip markdown
    for name, replacement in NAME_REPLACEMENTS.items():            # 4. phonetic names
        text = re.sub(rf"\b{re.escape(name)}\b", replacement, text)
    text = PAREN_REF_PATTERN.sub(convert_paren_ref, text)          # 5. parenthetical refs
    text = CHAPTER_RANGE_PATTERN.sub(convert_chapter_range, text)  # 6. inline chapter ranges
    text = REFERENCE_PATTERN.sub(convert_reference, text)          # 7. inline verse refs
    text = ensure_reference_periods(text)                          # 8. terminal periods on refs
    text = cleanup(text)                                           # 9. cleanup
    return text
