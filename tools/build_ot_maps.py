...
function initZoom(svg, FW, FH) {
  ...
  let drag=null, moved=false, pid=null;
  svg.addEventListener('pointerdown', e=>{ drag={x:e.clientX,y:e.clientY}; moved=false; pid=e.pointerId; });
  svg.addEventListener('pointermove', e=>{ if(!drag)return; const r=svg.getBoundingClientRect();
    if(Math.abs(e.clientX-drag.x)+Math.abs(e.clientY-drag.y)>4 && !moved){ moved=true; try{svg.setPointerCapture(pid);}catch(_){} }
    if(!moved) return;
  ...
  svg.closest('.mapbox').addEventListener('click', (e)=>{ if(!moved && !e.target.closest('.pt')) setMapInfo(svg.closest('.mapsec')); });
}
